...
class ImapClient {
    ...
    private fun readResp(input: InputStream, state: ImapState): String {
        ...
        while (true) {
            ...
            if (line.startsWith(tag) || line.startsWith("* BYE")) break
        }
        return sb.toString()
    }
    ...
    private fun decodeHeader(raw: String): String {
        return try {
            val r = Regex("""=\?([^?]+)\?([BbQq])\?([^?]*)\?=""")
            var last = 0; val sb = StringBuilder()
            r.findAll(raw).forEach { m ->
                sb.append(raw.substring(last, m.range.first)); last = m.range.last + 1
                ...
                sb.append(when (m.groupValues[2].uppercase()) {
                    ...
                    "Q" -> {
                        val bb = ByteArrayOutputStream(); var i = 0
                        while (i < txt.length) { when { txt[i] == '=' && i + 2 < txt.length -> { bb.write(txt.substring(i + 1, i + 3).toInt(16)); i += 3 } txt[i] == '_' -> { bb.write(' '.code); i++ } else -> { bb.write(txt[i].code); i++ } } }; String(bb.toByteArray(), cs) }
                    else -> txt
                })
            }; sb.append(raw.substring(last)); sb.toString()
        } catch (_: Exception) { raw }
    }

    private val dateFormats = listOf(SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss Z", Locale.US), SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss z", Locale.US), SimpleDateFormat("d MMM yyyy HH:mm:ss Z", Locale.US))
    ...
    private fun fetch(account: Account, page: Int, pageSize: Int): List<Email> {
        ...
        val socket = factory.createSocket(account.imapHost, 993) as SSLSocket
        ...
        try {
            ...
            if (total == 0) { sendCmd(output, "LOGOUT", state); readResp(input, state); return emptyList() }

            val end = total - page * pageSize
            val start = maxOf(1, end - pageSize + 1)
            if (start > end) { sendCmd(output, "LOGOUT", state); readResp(input, state); return emptyList() }

            sendCmd(output, "FETCH $start:$end (FLAGS BODY.PEEK[HEADER.FIELDS (SUBJECT FROM DATE)])", state)
            val hdrResp = readResp(input, state)

            val hdrBlocks = mutableMapOf<Int, String>(); var cs = 0; val cb = StringBuilder()
            for (line in hdrResp.split("\n")) {
                val fm = Regex("""\*\s+(\d+)\s+FETCH""").find(line)
                if (fm != null) { if (cs > 0) hdrBlocks[cs] = cb.toString(); cs = fm.groupValues[1].toIntOrNull() ?: 0; cb.clear(); cb.append(line) }
            ...
            for (s in start..end) {
                val h = hdrBlocks[s] ?: ""
                val uid = s.toLong()
                emails.add(Email(uid = uid, from = decodeHeader(findHdr(h, "From")), subject = decodeHeader(findHdr(h, "Subject")).ifBlank { "(无主题)" }, bodyPlain = "", bodyHtml = "", receivedDate = parseDate(findHdr(h, "Date")), isRead = h.contains("""\Seen"""), hasAttachments = false))
            }
